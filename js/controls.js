...
const initialTickerLength = Math.floor(window.innerWidth/8);
...
tickerMessageInput.value = "hello world";

let ledTicker1 = new Ticker(LEDTicker, initialTickerLength, Number(speedSlider.value), Number(sizeSlider.value), tickerMessageInput.value);
...
function updateTickerMessage(message) {
	...
	tickerMessageInput.value, {dotStyle: ledTicker1.dotStyle, dotOnStyle: ledTicker1.dotOnStyle});
	updateButtonListeners();
}
